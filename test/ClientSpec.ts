...
describe("API Client", () => {
	...
	/*
		afterEach(async () => {
			if (sitemapInfoData) {
				const deleteSitemapInfo: string = await client.deleteSitemap(sitemapInfoData.id);
...
*/
	...
	it("should get a sitemap", async () => {
		// sitemapInfoData = await client.createSitemap(sitemap);
		// const getSitemapInfo: IGetSitemapResponse =
		const response: IGetSitemapResponse = await client.getSitemap(408688);
		expect(response.id).to.be.equal(408688);
	 });
	...
	it("should update the sitemap", async () => {
		await client.updateSitemap(408688, sitemap);
	});

	it("should delete the sitemap", async () => {
		await client.deleteSitemap(408688);
	});
/*
